...
type Msg = mist.WebsocketMessage(SocketReq)
...
type Payload {
  Payload(
    resource: String, // e.g. `"Person"`, `List(Person)`
    json: String,
  )
}
...
fn init(
  ...
  ctx ctx: Context,
) -> #(Socket, Option(Selector(SocketReq))) {
  let self = process.new_subject()

  // let selector =
  //   process.new_selector()
  //   |> process.select_map(self, AppMsg)
  // #(Socket(self:, ctx:, conn:), Some(selector))

  #(Socket(self:, ctx:, conn:, state: State(people: [])), None)
}
...
fn update(
  ...
  case msg {
    ...
    mist.Text(json) -> {
      echo json
      // case json.parse(json, decoder_socket_req()) {
      //   // Ok(SocketReq(ref:, req:)) -> {
      //   Ok(msg) -> {
      //     socket.self
      //     |> process.send(mist.Custom(msg))
      //   }
    ...
    mist.Custom(msg) -> {
      case msg {
        SocketReq(ref:, req:) -> {
          let #(state, resp) = process(state: socket.state, req:)

          let msg = todo

          resp
          |> encode_resp(ref:, resp: _)
          |> json.to_string
...
fn handle(
  ...
  let #(state, resp) = process(state: socket.state, req: msg.req)

  resp
  |> encode_resp(ref: msg.ref, resp: _)
  |> json.to_string
  |> ws_send(conn: socket.conn, msg: _)
